...
const DIALS = {
  ...
  rainbow:{bg:0x1a1a1a, lume:0xc8a878, hand:0xc8a878, sec:0xc8a878, text:'#c8a878', surah:'Al-Insān', bezel:true, grainBlend:'soft-light', grainOpacity:0.05},
};
window.DIALS = DIALS;
...
const camDist = 380;
const camTiltDeg = 7; // bottom-up angle — like Nomos close-up product photography, makes recess geometry visible
...
function dialMat(color) {
  ...
  const special = {
    ...
    rainbow: { roughness:0.02, metalness:0.6, sheen:0, sheenColor:0x000000, sheenRoughness:0.8,
               clearcoat:1.0, clearcoatRoughness:0.005, envMapIntensity:4.0 }, // grand feu enamel — mirror-gloss black lacquer
  };
  ...
  const m = new THREE.MeshPhysicalMaterial({
    ...
    roughnessMap: s.clearcoat ? null : dialGrainTex,  // glossy dials skip grain texture
    clearcoat: s.clearcoat || 0.15,
    clearcoatRoughness: s.clearcoatRoughness || 0.3,
  });
  m.envMapIntensity = s.envMapIntensity || 0.6;
  ...
  for(let i=0;i<60;i++){
    ...
    if(isKawthar) {
      ...
      if(isHour && !isNumeralPos){
        ...
        const dotTopColor = c.dotTop || mk;
        const lumeMatl = new THREE.MeshPhysicalMaterial({
          color: dotTopColor, roughness: 0.06, metalness: 0.0,
          clearcoat: 1.0, clearcoatRoughness: 0.02,
          emissive: dotTopColor, emissiveIntensity: 0,
...
function buildNumerals() {
  ...
  for(const i of NUMERAL_POS){
    ...
    const numTopColor = c.numeralTop || mk;
    const topMatl = new THREE.MeshPhysicalMaterial({
      color: numTopColor, roughness: 0.06, metalness: 0.0,
      clearcoat: 1.0, clearcoatRoughness: 0.02,
      emissive: numTopColor, emissiveIntensity: 0,
      specularIntensity: 0.5,
    });
...
function buildQibla() {
  ...
  // Moon disc — grand feu enamel finish, ultra-glossy like polished porcelain
...
function onResize(){
  ...
  const targetScale = isFullscreen ? 0.57 : (CONTAINED ? 0.95 : (EMBED ? 0.65 : 0.57));
